...
for i in `seq 1 2`; do
  ...
  az network nic create     --resource-group VmRgName     --name webNici     --vnet-name quickVnet     --subnet quickVM_Snet3     --network-security-group quickNSG     --Location Location
done 
...
for i in `seq 1 2`; do
    ...
    az vm create         --admin-username quick         --resource-group VmRgName         --name webVMi         --nics webNici         --Location Location         --image Ubuntu2204         --availability-set quickAvailabilitySet         --generate-ssh-keys         --custom-data cloud-init.txt
done
...
echo '--------------------------------------------------------'
    ...
    az network public-ip create       --resource-group VmRgName       --Location Location       --alLocation-method Static       --name myPublicIP       --sku Standard
